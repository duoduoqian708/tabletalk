...
docker compose -f docker-compose.integration.yml up -d && CLEARED_INTEGRATION=1 .venv/bin/python -m pytest tests/integration -v   # PG/MySQL 集成测试（Docker 门控）
```
...
- 浏览器经 `GET /api/v1/bootstrap`（免鉴权）拿 `{token, dataDir}`，`src/renderer/src/hooks/useBootstrap.ts` 轮询注入 `src/renderer/src/api/client.ts`，请求自动带 `X-Cleared-Token`。
...
Config via env / `.env` (see `backend/.env.example`): `CLEARED_AI_PROVIDER` (mock|cloud|local), `CLEARED_AI_BASE_URL/API_KEY/MODEL`, `CLEARED_PORT`, `CLEARED_DATA_DIR`, `CLEARED_SIDECAR_TOKEN` (鉴权 token；缺省时启动生成并写入 `data_dir/sidecar.token`，浏览器经 `/bootstrap` 读取), `CLEARED_WEB_DIST` (SPA 构建目录，默认 `../frontend/dist`)。Runtime overridable via `PUT /api/v1/settings`.
...
backend/app/
  ...
  core/
    ...
    pool.py          PoolManager: per-connection 小池（SQLite=1 串行，PG/MySQL 默认 3，CLEARED_POOL_SIZE 可配）; run(fn) with auto-reconnect
...
- **`SELECT` without `LIMIT` gets `LIMIT (max_rows+1)` injected at the SQL layer** (`query._auto_cap`, `CLEARED_QUERY_MAX_ROWS` default 1000) so the *database* does less work, not just the transfer. `truncated` is inferred from the +1 row.
...
**Auth**: 中间件仅守 `/api/*`；除 `/health`、`/bootstrap` 与 OPTIONS 预检外，所有 `/api/*` 请求需带 `X-Cleared-Token` 头（token 见 `config.get_token()`）；静态资源 / SPA 免鉴权。`/bootstrap` 向浏览器发放 token，仅本机可访问（默认绑 127.0.0.1），未来网络化部署需加门禁。
...
- **Knowledge base semantic embedding**: retrieve 已是关键词+向量+图谱混合检索；真语义嵌入用 `CLEARED_EMBEDDING_PROVIDER=api` + `CLEARED_EMBEDDING_BASE_URL/MODEL/API_KEY`（任意 OpenAI 兼容 /embeddings，如本地 bge-m3），实现 `Embedder` 接口注入即可。
